...
class Permition(models.Model):
    permition_id = models.IntegerField(primary_key=True, db_column='permition_id')
    object_name = models.CharField(max_length=255, db_column='object_name')
    add = models.BooleanField(db_column='add')
    change = models.BooleanField(db_column='change')
    delete = models.BooleanField(db_column='del')
    visible = models.BooleanField(db_column='visible')
    group_id = models.ForeignKey('CGroup', on_delete=models.CASCADE, db_column='group_id', related_name='permissions')
