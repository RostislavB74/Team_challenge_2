...
class ShiftReportAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT SUM(amount) FROM dr_shift_report WHERE doc_id = @doc_id",
            ...
            if change:
                cursor.execute(
                    "SELECT draft FROM d_shift_report WHERE doc_id = @doc_id",
                    {"doc_id": int(obj.doc_id)},
                )
                ...
                cursor.execute(
                    """
                    EXEC SetShiftReport @doc_id, @doc_number, @description, @doc_date, @draft, @imported,
                                       @parent_doc_id, @production_line_id, @shift_id, @stoppage1,
                                       @kiln1, @comments1, @stoppage2, @kiln2, @comments2, @total,
                                       @begin_time, @end_time, @foreman_name
                    """,
                    {
                        "doc_id": int(obj.doc_id),
                        ...
                        "production_line_id": obj.production_line_id_id,
                        "shift_id": shift_id,
                        "stoppage1": obj.stoppage1,
                        "kiln1": obj.kiln1,
                        "comments1": obj.comments1,
                        "stoppage2": obj.stoppage2,
                        "kiln2": obj.kiln2,
                ...
                cursor.execute(
                    """
                    EXEC AddShiftReport @doc_id OUTPUT, @doc_num OUTPUT, @description, @doc_date, @draft,
                                       @imported, @parent_doc_id, @production_line_id, @shift_id,
                                       @stoppage1, @kiln1, @comments1, @stoppage2, @kiln2, @comments2,
                                       @total, @begin_time, @end_time, @foreman_name
                    """,
                    {
                        "description": obj.description,
                        "doc_date": now,
    ...
    def save_related(self, request, form, formsets, change):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT SUM(amount) FROM dr_shift_report WHERE doc_id = @doc_id",
                {"doc_id": int(form.instance.doc_id)},
            )
            total = cursor.fetchone()[0] or 0
            cursor.execute(
                "SELECT draft FROM d_shift_report WHERE doc_id = @doc_id",
            ...
            cursor.execute(
                ...
                {
                    "doc_id": int(form.instance.doc_id),
                    "doc_number": form.instance.doc_number,
                    "description": form.instance.description,
                    "doc_date": form.instance.doc_date,
                    "draft": draft,
                    "imported": form.instance.imported,
                    "parent_doc_id": form.instance.parent_doc_id,
                    "production_line_id": form.instance.production_line_id_id,
                    "shift_id": form.instance.shift_id_id,
                    "stoppage1": form.instance.stoppage1,
                    "kiln1": form.instance.kiln1,
                    "comments1": form.instance.comments1,
                    "stoppage2": form.instance.stoppage2,
                    "kiln2": form.instance.kiln2,
                    "comments2": form.instance.comments2,
                    "total": total,
                    "begin_time": form.instance.begin_time,
                    "end_time": form.instance.end_time,
                    "foreman_name": form.instance.foreman_name,
                },
            )
    ...
    def delete_model(self, request, obj):
        with connection.cursor() as cursor:
            ...
            if not draft and not request.user.is_superuser:
                ...
                return
            cursor.execute("EXEC DelShiftReport @doc_id", {"doc_id": int(obj.doc_id)})
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("EXEC GetShiftReportDoc @doc_id", {"doc_id": int(doc_id)})
            ...
            logger.debug(f"Production line ID: {production_line_id}")

            # Отримання назви лінії через SQL
            logger.debug(
                f"Fetching line name for production_line_id={production_line_id}"
            )
            cursor.execute(
                "SELECT production_line FROM c_production_line WHERE production_line_id = @prod_id",
                {"prod_id": int(production_line_id)},
            )
            result = cursor.fetchone()
            if result:
                line_name = (
                    result[0].strip() if isinstance(result[0], str) else str(result[0])
                )
                shift_doc["line_name"] = line_name
                logger.debug(
                    f"Line name for production_line_id={production_line_id}: {line_name}"
                )
            else:
                shift_doc["line_name"] = f"Лінія {production_line_id}"
                logger.warning(
                    f"No production_line found for production_line_id={production_line_id}"
                )
            ...
            cursor.execute(
                ...
                SELECT dr2.row_id, ck.kiln AS kiln_name, dr2.amount, dr2.stoppage_cause_type_id, dr2.stoppage_cause, dr2.comment,
                       CONCAT('Тип: ', CAST(dr2.stoppage_cause_type_id AS VARCHAR), ', Причина: ', dr2.stoppage_cause, ', Опис: ', ISNULL(dr2.comment, '')) AS downtime_summary
                FROM dr2_shift_report dr2
                INNER JOIN cu_kiln ck ON dr2.kiln_id = ck.kiln_id
                WHERE dr2.doc_id = @doc_id
            """,
                {"doc_id": int(doc_id)},
            )
            ...
            for downtime in downtimes:
                logger.debug(f"Downtime: {downtime}")
            extra_context["downtimes"] = downtimes

            # Отримання рядків звіту
            cursor.execute("EXEC GetShiftReportRows @doc_id", {"doc_id": int(doc_id)})
            columns = [col[0] for col in cursor.description]
